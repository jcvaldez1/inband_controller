...
def parse_results(host_count=0, filename="", container_count=0):
    ...
    included_groups = [120,160,200]
    #for fname in range(0,101,10):
    for fname in included_groups:
        try:
              actuator_dir = str(container_count)+"_pair_even/"+str(fname)+'_dev/actuator/52.74.73.2/'
              receiver_dir = str(container_count)+"_pair_even/"+str(fname)+'_dev/receiver/13.55.147.1/'
              ...
              plt.plot(x_axis, y_axis, label = str(int(fname)/2)+" simultaneous device pairs") 
    ...
    plt.title('Delay for each request on the First device pair ('+str(container_count)+' container pairs)') 
